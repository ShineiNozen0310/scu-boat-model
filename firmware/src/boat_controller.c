...
void BoatController_Init(BoatController *controller)
{
    ...
    controller->has_seen_command = false;
}
...
bool BoatController_HandleCommand(BoatController *controller, uint8_t ir_command, uint32_t now_ms)
{
    ...
    controller->has_seen_command = true;
    return changed;
}
...
void BoatController_Tick(BoatController *controller, uint32_t now_ms, const BoatHal *hal)
{
    ...
    if (controller->has_seen_command &&
        (uint32_t)(now_ms - controller->last_command_ms) > BOAT_COMMAND_TIMEOUT_MS)
    {
        controller->throttle_target_percent = 0U;
        controller->has_last_repeatable_command = false;
    }
